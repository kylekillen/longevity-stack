...
import type { CartData, CartProduct } from "./generateCartData";
...
function CartContent({ data }: { data: CartData }) {
  const searchParams = useSearchParams();
  const itemSlugs = searchParams.get("items")?.split(",").filter(Boolean) || [];
  const vendorLock = searchParams.get("vendor") || undefined;
  ...
  const products: (CartProduct | null)[] = itemSlugs.map((slug) => {
    if (vendorLock && data.productsByVendor[slug]?.[vendorLock]) {
      return data.productsByVendor[slug][vendorLock];
    }
  ...
  const allInteractions = ingredientSlugs
    .map(slug => data.interactions[slug])
    .filter(Boolean);

  // Find cross-ingredient synergies
...
export default function CartClient({ data }: { data: CartData }) {
  return (
    <Suspense fallback={
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <h1 className="text-3xl font-bold text-[var(--foreground)] mb-4">Loading your stack...</h1>
      </div>
    }>
