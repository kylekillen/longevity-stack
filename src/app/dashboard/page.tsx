      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <p className="text-sm text-[var(--gray)] mt-0.5">{user.email}</p>
          </div>
          <Link
            href="/intake"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-[var(--gray)] uppercase tracking-wide mb-2">Active prescriptions</p>
            <p className="text-2xl font-bold text-[var(--navy)]">{activeCount || "—"}</p>
            <p className="text-xs text-[var(--gray-light)] mt-0.5">
              {activeCount > 0 ? `${activeCount} active subscription${activeCount > 1 ? "s" : ""}` : "Complete intake to start"}
            </p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-[var(--gray)] uppercase tracking-wide mb-2">Physician review</p>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-[var(--gray)] uppercase tracking-wide mb-2">Account</p>
            <p className="text-2xl font-bold text-[var(--navy)]">Active</p>
            <p className="text-xs text-[var(--gray-light)] mt-0.5">Member since {new Date(user.created_at).toLocaleDateString("en-US", { month: "short", year: "numeric" })}</p>
          </div>
        </div>
        ...
        {subscriptions && subscriptions.length > 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="font-semibold text-[var(--navy)]">Your subscriptions</h2>
            </div>
            <div className="divide-y divide-gray-100">
              {subscriptions.map((sub) => (
                <div key={sub.id} className="px-6 py-4 flex items-center justify-between">
                  <div>
                    <p className="font-medium text-[var(--navy)] capitalize">{sub.product_slug.replace(/-/g, " ")}</p>
